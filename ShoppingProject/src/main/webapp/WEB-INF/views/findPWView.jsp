	...
	//확인
		$("#check").click(function() {	
			...
			if(id.length == 0){
				alert("아이디를 입력해주세요.");	
				return false;
			}else if(newmail.length == 0 || newmail2.length == 0 || selectmail.length == 0){
				alert("이메일을 입력해주세요.");
				return false;
			//validateion 체크
			}else if(regmail.test(email) != true){
  ...
  <div class="container">
    <div class="row"> 
     ...
     <div class="card-body"> 
		 ...
		 <form action="${contextPath}/board/findPW" method="POST" id="emailForm">     	
             	<div style="margin-top: 20px;padding: 20px;border: 1px solid #dae5eb;background-color: #e9f4fa;text-align:center;padding-bottom:4px">
		            <p class="secession-validate-user-msg" style="margin-bottom: 10px">
		                <strong>아이디와 가입할때 입력하신 이메일을 입력해주세요.</strong>
		            </p> 
		            <label>
		            	 id:             		               
		                ...
		                email:             		               
								...
								<select  
						   		...
						   		<input type="hidden" id="email" name="email" value="${member.email}" />	
				        				...
				        				<button id="check" name="check" class="btn-check"
				                          style="    width: 116px;
										    padding: 5px 3px 3px;
										    margin-bottom: 7px;
										    font-size: 12px;
