			...
			<div class="s_wrap">
				...
				<div class="tbl_frm01">
					...
					<tr>
						...
						<td>
							<select id="searchType" name="searchType">
								<option value="O.buynum">주문번호</option> 
								<option value="M.membernum">회원번호</option>
					...
					<tr>
						...
						<td>
							<select name="dateType">
								...
								<option value="deliveryday">배송예정일</option>
								<option value="deliverycom">배송완료일</option>
							</select>
					...
					<tr>
						...
						<td>
							...
							<label><input type="radio" name="state" value="1"> 배송완료</label>
							<label><input type="radio" name="state" value="2"> 배송중</label>
							<label><input type="radio" name="state" value="3"> 주문확인중</label>
							<label><input type="radio" name="state" value="4"> 주문취소</label>
							<label><input type="radio" name="state" value="8"> 구매확정</label>
						</td>
					</tr>
					</tbody>
					</table>
				...
				<div class="tbl_head01">
					<table id="myTable" class="table table-bordered table-striped table-hover table-responsive" table-layout: auto;">
						...
						<thead>
							<tr>
						        <th scope="col" onclick="sortTable(0)" class="active" data-sort="number">주문번호</th>
						        <th scope="col" onclick="sortTable(1)" class="active">회원번호</th>
						        <th scope="col" onclick="sortTable(2)" class="active">회원아이디</th>
						        <th scope="col" onclick="sortTable(3)" class="active">결제방법</th>
						        <th scope="col" onclick="sortTable(4)" class="active">결제금액</th>
						        <th scope="col" onclick="sortTable(5)" class="active">주문일</th>
						        <th scope="col" onclick="sortTable(6)" class="active">배송예정일</th>
						        <th scope="col" onclick="sortTable(7)" class="active">배송완료일</th>
    ...
    <script>
	    ...
	    $(function(){
	    	$("#searchButton").click(function(){
		        ...
		        $.ajax({
		            url: "${contextPath}/board/adminOrderSearch",
		            type: "get",
		            data: formData,
		            dataType: "json",
		            ...
		            success: function (response) {
		                ...
		                for (var i = 0; i < response.searchResult.length; i++) {
		                	var adminOrder = response.searchResult[i];
		                    ...
		                    $("<td>").text(adminOrder.deliverycom ? adminOrder.deliverycom : '').appendTo(row);
		                    $("<td>").text(adminOrder.state_nm).appendTo(row);
		                    // 클릭 시 회원 상세 페이지로 이동
		                    row.on("click", function () {
		                        var buynum = adminOrder.buynum;
		                        window.location.href = "${contextPath}/board/adminOrderDetail?buynum=" + buynum;
		                    });
